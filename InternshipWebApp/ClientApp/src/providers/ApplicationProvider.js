...
export const ApplicationProvider = (props) => {
  ...
  useEffect(() => {
    userManager.events.addUserLoaded((user) => {
      ...
      axios
        .get(process.env.REACT_APP_AUTH_URL + "/api/account/icon", {
          responseType: "blob",
          headers: {
        ...
        .then((response) => {
          let imageData = new Blob([response.data], {
          ...
          reader.onloadend = function () {
            ...
            dispatch({
              type: SET_ICON,
              icon: base64data,
              iconType: response.headers["content-type"],
            });
          };
    ...
    userManager.events.addUserUnloaded(() => {
      dispatch({
        type: ADD_MESSAGE,
        text: "Informace o přihlášení jsou neplatné.",
        variant: "info",
        dismissible: true,
        expiration: 3,
      });
    ...
    userManager.events.addAccessTokenExpiring(() => {
      dispatch({
        type: ADD_MESSAGE,
        text: "Platnost Vašeho přihlášení brzy vyprší.",
        variant: "warning",
        dismissible: true,
        expiration: 10,
      });
    ...
    userManager.events.addAccessTokenExpired(() => {
      dispatch({
        type: ADD_MESSAGE,
        text: "Platnost Vašeho přihlášení vypršela.",
        variant: "info",
        dismissible: true,
        expiration: 10,
      });
    ...
    userManager.events.addSilentRenewError(() => {
      dispatch({
        type: ADD_MESSAGE,
        text: "Nepodařilo se obnovit platnost Vašeho přihlášení.",
        variant: "error",
        dismissible: true,
        expiration: false,
      });
    ...
    userManager.events.addUserSignedOut(() => {
      dispatch({
        type: ADD_MESSAGE,
        text: "Uživatel byl automaticky odhlášen.",
        variant: "warning",
        dismissible: true,
        expiration: 3,
      });
